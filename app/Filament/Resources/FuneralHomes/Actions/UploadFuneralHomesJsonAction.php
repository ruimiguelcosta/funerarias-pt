...
namespace App\Filament\Resources\FuneralHomes\Actions;
...
class UploadFuneralHomesJsonAction
{
        ...
        return Action::make('upload_json')
            ...
            ->action(function (array $data): void {
                try {
                    ...
                    $funeralHomesData = json_decode($jsonContent, true);

                    if (json_last_error() !== JSON_ERROR_NONE) {
                        throw new \Exception('Ficheiro JSON inválido: '.json_last_error_msg());
                    }

                    if (! is_array($funeralHomesData)) {
                    ...
                    DB::transaction(function () use ($funeralHomesData, &$importedCount, &$updatedCount, &$errors) {
                        foreach ($funeralHomesData as $index => $data) {
                            try {
                                $existingFuneralHome = FuneralHome::query()
                                    ->where('place_id', $data['placeId'] ?? null)
                                    ->orWhere('title', $data['title'] ?? null)
                                    ->first();

                                $processedData = self::processFuneralHomeData($data);

                                if ($existingFuneralHome) {
                                    $existingFuneralHome->update($processedData);
                                    $updatedCount++;
                                } else {
                                    FuneralHome::query()->create($processedData);
                                    $importedCount++;
                                }
                            } catch (\Exception $e) {
                                $errors[] = 'Linha '.($index + 1).': '.$e->getMessage();
                                Log::error('Erro ao processar funerária', [
                                    'index' => $index,
                                    'data' => $data,
                                    'error' => $e->getMessage(),
                                ]);
    ...
    private static function processFuneralHomeData(array $data): array
    {
        return [
