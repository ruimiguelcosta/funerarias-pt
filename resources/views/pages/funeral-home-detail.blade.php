...
<div class="pt-20">
    ...
    <x-breadcrumbs :items="[
        ...
        ['label' => $entity->city, 'url' => route('city-funeral-homes', $entity->city_slug)],
        ['label' => $entity->title]
    ]" />

    <!-- Hero Section -->
    <div class="h-[400px] bg-cover bg-center relative"
         style="background-image: url('{{ $entity->images->where('category', 'main')->first()?->local_url ?? $entity->images->first()?->local_url ?? 'https://images.unsplash.com/photo-1584907797015-7554cd315667?w=800&h=400&fit=crop' }}')">
            ...
            <div class="max-w-5xl mx-auto">
                <div class="flex justify-between items-start mb-8">
                    <div>
                        ...
                        @if($entity->sub_title)
                        <p class="text-lg text-gray-600">{{ $entity->sub_title }}</p>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-2 bg-yellow-100 px-4 py-2 rounded-full">
                        ...
                        <span class="text-xl font-bold text-yellow-800">{{ $entity->total_score }}</span>
                        @if($entity->reviews_count)
                        <span class="text-sm text-gray-600">({{ $entity->reviews_count }} avaliações)</span>
                        @endif
                    </div>
                    @endif
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600">{{ $entity->postal_code }} {{ $entity->city }}</p>
                                @endif
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <a href="{{ $entity->website }}" target="_blank" class="text-purple-600 hover:text-purple-800">{{ $entity->website }}</a>
                            </div>
                        </div>
                        @endif
                ...
                @if($entity->generated_description)
                <div class="bg-white rounded-lg border border-gray-200 mb-8 p-6">
                    <h2 class="font-playfair text-2xl text-purple-700 mb-4">
                ...
                <!-- Reviews Section -->
                <x-reviews-section :reviews="$entity->reviews" :entity="$entity" />

                <div class="mt-12 text-center">
                    <a href="tel:{{ $entity->phone }}" class="bg-gradient-to-r from-purple-600 to-purple-500 text-white px-8 py-4 rounded-lg text-lg font-semibold hover:opacity-90 transition-all duration-300 inline-block">
                        Contactar Agora
                    </a>
                </div>
            </div>
